...
| `landControle_directories` | list[string] | One or more LandUseControl / mask directories to apply (can stack multiple controls). |
| `aft_production_directory` | string | AFT production definitions directory (default production tables). |
| `aft_behevoir_directory` | string | AFT behaviour definitions directory (agent parameters). |
| `service_demands_path` | string | Demand file or demand directory for services (world or region-aware). |
| `service_utility_weight_path` | string | Service utility weights file (or directory, depending on template). |
| `services_taxes_subsidies_path` | string | Service-level taxes/subsidies inputs (file or directory). |
| `land_taxes_subsidies_path` | string | AFT/land-level taxes/subsidies inputs (often region-aware). |
| `capital_degradation_directory` | string | Folder with degradation/shock maps applied to capitals (often year-indexed). |
| `waitingFlag_directories_path` | string | Path to a waiting-flags definition file (used only if coupling/waiting updater is enabled). |
| `GIS_data_path` | string | Optional GIS attributes path (regions, auxiliary spatial fields) if your dataset separates it. |
| `Behevoir_Cells_directory` | string | Optional behaviour cells directory (project-specific feature). |
...
| Key | Type | Meaning |
|---|---:|---|
| `regionalization` | boolean | If `true`, build multiple regions (requires region identifiers + region-aware inputs such as demands). If `false`, run as one “world” region. |
...
> If you enable `regionalization` but only see “world” behaviour, check logs for missing regional demands/region IDs.
...
| `use_AFTs_categories_GiveIn` | boolean | If enabled, give-in logic can be constrained by AFT categories (requires categories defined for each AFT). |
| `mutate_on_competition_win` | boolean | If `true`, a winning competitor may create a mutated “child” AFT instance on takeover (evolution-style runs). |
| `mutation_interval` | number | Mutation strength/interval used when `mutate_on_competition_win` is enabled. |
...
| Key | Type | Meaning |
|---|---:|---|
| `use_neighbor_priority` | boolean | If `true`, the model can derive a candidate competitor set from an extended Moore neighbourhood but still use random . |
| `neighbor_radius` | integer | Neighbourhood radius (in cells) used when neighbour effects are enabled. |
| `neighbor_priority_probability` | number | Probability of applying neighbour priority logic. |

---

## 8) Competition, seeding, and rates

### 8.1 Seeding strategy: `seedID`

| Key | Type | Meaning |
|---|---:|---|
| `seedID` | string/number | Controls how “participating cells” are selected each tick for processes like competition/abandonment. |

Common patterns:
- `seedID: rank`  
  Deterministic ranking-based selection (select lowest-utility cells up to the chosen percentage).
- `seedID: 1234`  
  Reproducible pseudo-random selection using that seed.
- `seedID: /path/or/directory`  
  Project-specific mode: read seed cells from file(s), if supported.

### 8.2 Process rates and knobs

| Key | Type | Range | Meaning |
|---|---:|---:|---|
| `land_abandonment_percentage` | number | 0..1 | Max fraction of cells that can abandon land per tick (year). |
| `participating_cells_percentage` | number | 0..1 | Fraction of cells participating in the competitiveness/competition process per tick. |
| `takeOverUnmanageCells_percentage` | number | 0..1 | Fraction of unmanaged/abandoned cells considered for take-over. |
| `MostCompetitorAFTProbability` | number | 0..1 | Probability of choosing the best-performing competitor (the Highest utility); otherwise a random competitor is tested. |
| `marginal_utility_calculations_per_tick` | integer | ≥1 | How many times marginal utility is recalculated per tick. |
...
| Key | Type | Meaning |
|---|---:|---|
| `generate_charts_plots_PNG` | boolean | Export charts as PNG (if chart exporters are enabled). |
| `generate_charts_plots_PDF` | boolean | Export charts as PDF (if PDF exporters are enabled). |
...
| `generate_map_plots_tif` | boolean | Export GeoTIFF maps (if supported/installed). |
...
| `printAbstractModelRunnerMeasures` | boolean | Print timing/performance measures from the abstract runner. |
| `printRegionalModelRunnerMeasures` | boolean | Print timing/performance measures from the regional runner. |
