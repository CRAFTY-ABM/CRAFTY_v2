...
public final class LlmPolicyParser {
	...
	public static String onlyWhenUnparseableOutput(String unparseableOutput, Collection<String> expectedPolicyKeys) {
		List<String> keys = (expectedPolicyKeys == null ? List.<String>of()
				: expectedPolicyKeys.stream().filter(Objects::nonNull).map(String::trim).filter(s -> !s.isEmpty())
		...
		return "AUTOMATIC PARSER ERROR (Java):\r\n" + "Your last response was not parseable JSON.\r\n" + "\r\n"
				...
				+ "- policy_decisions values must be numbers (no quotes), finite.\r\n"
	...
	public static String promptModifierToForceFormat(String originalPrompt, Collection<String> expectedPolicyKeys) {
		...
		String strictHeader = "AUTOMATION NOTICE (Java JSON parser; no human supervision):\n"
