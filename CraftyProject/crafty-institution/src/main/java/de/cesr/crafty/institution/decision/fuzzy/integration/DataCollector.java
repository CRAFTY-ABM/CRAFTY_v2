...
public class DataCollector {
	...
	static FuzzyConnector connector;
	...
	public static void init(FuzzyConnector connector) {
	...
	private static void outputServicesTxSuCsv() {
		...
		for (int i = 0; i < ServiceSet.getServicesList().size(); i++) {
			...
			double t = CellsLoader.hashCell.values().stream()
					.mapToDouble(cell -> cell.getServicesTax().getOrDefault(serviceName, 0.0)).average().orElse(0);
	...
	private static void outputLandTxSuCsv() {
		...
		for (int i = 1; i < aftTxSu[0].length; i++) {
			String aftName = aftTxSu[0][i];
			double t = CellsLoader.hashCell.values().stream()
					.filter(cell -> cell.getOwner() != null && aftName.equals(cell.getOwnerName()))
					.mapToDouble(cell -> cell.getLandTax().getOrDefault(aftName, 0.0)).average().orElse(0);
			aftTxSu[iteration][i] = String.valueOf(t);
		}
